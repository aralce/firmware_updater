...
// ----------------------------------------------------- AP function -----------------------------------------------------
// -----------------------------------------------------------------------------------------------------------------------
const char WIFI_AP_TAG[8] = "WiFi AP";
...
bool WiFiAPClass::softAP(const char* ssid, const char* passphrase, int channel, int ssid_hidden, int max_connection, bool ftm_responder)
{
    ...
    if(!ssid || *ssid == 0) {
        // fail SSID missing
        ESP_LOGE(WIFI_AP_TAG, "SSID missing!");
        // log_e("SSID missing!");
        return false;
    }
    ...
    if(passphrase && (strlen(passphrase) > 0 && strlen(passphrase) < 8)) {
        ...
        ESP_LOGE(WIFI_AP_TAG, "passphrase too short!");
    ...
    if(!WiFi.enableAP(true)) {
        // enable AP failed
        ESP_LOGE(WIFI_AP_TAG, "enable AP first!");
    ...
    if(err){
    	ESP_LOGE(WIFI_AP_TAG, "get AP config failed");
    ...
    if(!softap_config_equal(conf, conf_current)) {
        ...
        if(err){
        	ESP_LOGE(WIFI_AP_TAG, "set AP config failed");
...
IPAddress WiFiAPClass::softAPIP()
{
    ...
    if(esp_netif_get_ip_info(get_esp_interface_netif(ESP_IF_WIFI_AP), &ip) != ESP_OK){
    	ESP_LOGE(WIFI_AP_TAG, "Netif Get IP Failed!");
...
const char * WiFiAPClass::softAPgetHostname()
{
    ...
    if(esp_netif_get_hostname(get_esp_interface_netif(ESP_IF_WIFI_AP), &hostname) != ESP_OK){
    	ESP_LOGE(WIFI_AP_TAG, "Netif Get Hostname Failed!");
