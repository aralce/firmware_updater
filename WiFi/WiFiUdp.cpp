...
const char WIFI_UDP_TAG[9] = "Wifi UDP";
...
uint8_t WiFiUDP::begin(IPAddress address, uint16_t port){
  ...
  if(!tx_buffer){
    ESP_LOGE(WIFI_UDP_TAG, "could not create tx buffer: %d", errno);
  ...
  if (setsockopt(udp_server,SOL_SOCKET,SO_REUSEADDR,&yes,sizeof(yes)) < 0) {
      ESP_LOGE(WIFI_UDP_TAG, "could not set socket option: %d", errno);
  ...
  if(bind(udp_server , (struct sockaddr*)&addr, sizeof(addr)) == -1){
    ESP_LOGE(WIFI_UDP_TAG, "could not bind socket: %d", errno);
  ...
  if(begin(IPAddress(INADDR_ANY), p)){
    if(a != 0){
      ...
      if (setsockopt(udp_server, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0) {
          ESP_LOGE(WIFI_UDP_TAG, "could not join igmp: %d", errno);
...
int WiFiUDP::beginPacket(){
  ...
  if ((udp_server=socket(AF_INET, SOCK_DGRAM, 0)) == -1){
    ESP_LOGE(WIFI_UDP_TAG, "could not create socket: %d", errno);
    // log_e("could not create socket: %d", errno);
    return 0;
  }
...
int WiFiUDP::beginPacket(const char *host, uint16_t port){
  ...
  if (server == NULL){
    ESP_LOGE(WIFI_UDP_TAG, "could not get host from dns: %d", errno);
...
int WiFiUDP::endPacket(){
  ...
  if(sent < 0){
    ESP_LOGE(WIFI_UDP_TAG, "could not send data: %d", errno);
...
int WiFiUDP::parsePacket(){
  ...
  if ((len = recvfrom(udp_server, buf, 1460, MSG_DONTWAIT, (struct sockaddr *) &si_other, (socklen_t *)&slen)) == -1){
    ...
    ESP_LOGE(WIFI_UDP_TAG, "could not receive data: %d", errno);
