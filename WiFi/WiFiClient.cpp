...
const char WIFI_CLIENT[12] = "WiFi Client";
...
private:
        ...
        size_t fillBuffer()
        {
            if(!_buffer){
                ...
                if(!_buffer) {
                    ESP_LOGE(WIFI_CLIENT, "Not enough memory to allocate buffer");
...
int WiFiClient::connect(IPAddress ip, uint16_t port, int32_t timeout_ms)
{
    ...
    if (sockfd < 0) {
        ESP_LOGE(WIFI_CLIENT, "socket: %d", errno);
    ...
    if (res < 0 && errno != EINPROGRESS) {
        ESP_LOGE(WIFI_CLIENT, "connect on fd %d, errno: %d, \"%s\"", sockfd, errno, strerror(errno));
    ...
    if (res < 0) {
        ESP_LOGE(WIFI_CLIENT, "select on fd %d, errno: %d, \"%s\"", sockfd, errno, strerror(errno));
        // log_e("select on fd %d, errno: %d, \"%s\"", sockfd, errno, strerror(errno));
        close(sockfd);
        return 0;
    } else if (res == 0) {
        ESP_LOGI(WIFI_CLIENT, "select returned due to timeout %d ms for fd %d", _timeout, sockfd);
        ...
        if (res < 0) {
            ESP_LOGE(WIFI_CLIENT, "getsockopt on fd %d, errno: %d, \"%s\"", sockfd, errno, strerror(errno));
        ...
        if (sockerr != 0) {
            ESP_LOGE(WIFI_CLIENT, "socket error on fd %d, errno: %d, \"%s\"", sockfd, sockerr, strerror(sockerr));
...
#define ROE_WIFICLIENT(x,msg) { if (((x)<0)) { ESP_LOGE(WIFI_CLIENT, "Setsockopt '" msg "'' on fd %d failed. errno: %d, \"%s\"", sockfd, errno, strerror(errno));/**log_e("Setsockopt '" msg "'' on fd %d failed. errno: %d, \"%s\"", sockfd, errno, strerror(errno))**/; return 0; }}
...
int WiFiClient::setSocketOption(int level, int option, const void* value, size_t len)
{
    ...
    if(res < 0) {
        ESP_LOGE(WIFI_CLIENT, "fail on %d, errno: %d, \"%s\"", fd(), errno, strerror(errno));
...
int WiFiClient::getOption(int option, int *value)
{
    ...
    if(res < 0) {
        ESP_LOGE(WIFI_CLIENT, "fail on fd %d, errno: %d, \"%s\"", fd(), errno, strerror(errno));
    ...
    if (_connected) {
        ...
        if (res <= 0){
          switch (errno) {
              ...
              case ECONNABORTED:
                  _connected = false;
                  ESP_LOGD(WIFI_CLIENT, "Disconnected: RES: %d, ERR: %d", res, errno);
                //   log_d("Disconnected: RES: %d, ERR: %d", res, errno);
                  break;
              default:
                  ESP_LOGI(WIFI_CLIENT, "Unexpected: RES: %d, ERR: %d", res, errno);
